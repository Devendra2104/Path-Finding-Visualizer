...
function resetCanvas() {
  ...
  if (source === undefined || destination === undefined) {
    ...
    x =
      Math.floor(Math.random() * (cols - Math.floor(cols / 2 + 1))) +
      Math.floor(cols / 2 + 1);
    y = Math.floor(Math.random() * rows);
...
function draw() {
  if (started) {
    ...
    for (path of shortestPath) {
      vertex(
        path.i * resolution + resolution / 2,
        path.j * resolution + resolution / 2
      );
    }
    endShape();
    source.show(color(87, 50, 168));
...
function mouseDragged() {
  ...
  for (let i = 0; i < cols; i++) {
    for (let j = 0; j < rows; j++) {
      ...
      if (
        mouseX >= graph[i][j].x &&
        mouseX <= graph[i][j].x + graph[i][j].r &&
        mouseY >= graph[i][j].y &&
        mouseY <= graph[i][j].y + graph[i][j].r
      ) {
        console.log("in IF");
        if (graph[i][j] != source && graph[i][j] != destination) {
          graph[i][j].clicked();
